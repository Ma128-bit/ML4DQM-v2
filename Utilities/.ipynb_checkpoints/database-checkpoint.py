...
def update_step(job_id, step_name, output_path):
    """
    Update the output path of a specific step (S1–S5) for the given job_id.
...
def clone_job_with_steps(from_job_id, steps_to_copy):
    ...
    created_at = datetime.utcnow().isoformat()
    insert_cols = "MEname, job_id, created_at" + (", " + cols if steps_to_copy else "")
    placeholders = ", ".join(["?"] * (3 + len(steps_to_copy)))
    c.execute(f"""
        INSERT INTO {TABLE_NAME} ({insert_cols})
        VALUES ({placeholders})
    """, (MEname, new_job_id, created_at, *values))
    conn.commit()
    conn.close()
