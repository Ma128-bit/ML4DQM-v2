...
def train(df: pd.DataFrame, job_label: str, data_name: str, batch_size: int = 64, epochs: int = 50, learning_rate: float = 2.e-4, patience: int = 4, threshold: float = 0.005) -> torch.jit.ScriptModule:
    df = df[df["bad_flag"]==1]
    ...
    scripted_model.save(f"{job_label}/trained_model_{device}.pth")
    if torch.cuda.is_available():
        scripted_model.to("cpu").save(f"{job_label}/trained_model_cpu.pth")
...
def predictions(df: pd.DataFrame, model: torch.jit.ScriptModule, data_name: str, num_sectors: int) -> pd.DataFrame:
    ...
    from custom_CSC_sel import rebin_image
